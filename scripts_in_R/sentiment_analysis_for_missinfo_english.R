...
data <- read_xlsx("Govgr_Covid19_Misinfo_english.xlsx")
...
df_review_emotion_sums <- data.frame(Emotions = c("Anger", "Anticipation", "Disgust", "Fear", "Joy", "Sadness", "Surprise", "Trust", "Negativeness", "Positiveness"),
...
ggplot(df_review_emotion_sums_2[-10,],aes(x= `Emotions`, y=`Sum`, fill = `Emotions`))+
  ...
  ggtitle("Sentiment Analysis of Misinformation Sentences") +
  xlab('Emotions')+
  ylab('Frequency')+
  theme(
...
ggplot(subject, aes(y= `subject`, x=n, fill = `subject`))+
  geom_bar(stat='identity', position = 'dodge')+
  ggtitle("Subject of Misinformation Sentences") +
  theme(
    plot.title = element_text(hjust = 0.5)
  )+
...
ggplot(aim, aes(y= `aim`, x=n))+
  ...
  ggtitle("Target of Misinformation Sentences") +
  xlab("n") +
  ylab("")+
...
ggplot(misinfo_time, aes(x=`date`, y=n))+
  ...
  ggtitle("Chronological Distribution of Misinformation Sentences’ Records on covid19.gov.gr") +
  xlab("Date") +
  ylab("n") +
  ylim(-1,20)+
  geom_text(aes(label = n), position = position_dodge(width = 0.9), vjust = -0.5, colour = "black", size = 7)+
...
g7 <- ggplot(data_for_links, aes(x= `Sources`, fill = `Sources` ))+
  ...
  ggtitle("Distribution of Sources of Covid-19 Protection Measures in Greece") +
  theme(
    plot.title = element_text(hjust = 0.5)
  )+
